...
export const brandSchema = z.object({
  ...
  website: z.string().min(0).refine( // Change from optional to required empty string
    (val) => !val || z.url().safeParse(val).success,
    "Please enter a valid website URL"
  ),
  isActive: z.boolean(),
  phone: z.string().min(0).refine( // Change from optional to required empty string
...
export function safeValidateFormData<T>(schema: z.ZodSchema<T>, data: unknown): T | null {

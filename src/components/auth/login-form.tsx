...
export function LoginForm() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const rememberedUsername = localStorage.getItem('rh-pos-remember-username')
    ...
    login(credentials, {
      onSuccess: () => {
        ...
        if (rememberMe) {
          ...
          if (typeof window !== 'undefined') {
            localStorage.setItem('rh-pos-remember-username', credentials.username)
          }
        } else {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                {process.env.NODE_ENV === 'development' && (
                  <div className="mt-4 p-3 bg-blue-50 rounded-md border border-blue-200">
                    ...
                    <p className="text-xs text-blue-600">Password: <code className="bg-blue-100 px-1 rounded">Admin@123!</code></p>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <div className="text-center">
              <Link href="/auth/forgot" className="text-sm text-primary hover:text-primary/80 font-medium">
            ...
            <Button type="submit" className="w-full" size="lg" disabled={isPending}>
              {isPending ? t('auth.login.signingIn') : t('auth.login.signInButton')}
            </Button>
          </form>
        </Form>

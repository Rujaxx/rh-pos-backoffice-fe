...
export function DataTable<T extends Record<string, unknown>>({
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((item) => {
      return columns.some((column) => {
        ...
        const value = column.accessor ? item[column.accessor] : item[column.id as keyof T]
  ...
  const handleSort = (columnId: string) => {
    ...
    if (onSort) {
      onSort(columnId, sortColumn === columnId && sortDirection === "asc" ? "desc" : "asc")
    }
  }
          ...
          <TableBody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((item, index) => (
                <TableRow key={(item._id as string) || (item.id as string) || index}>
                  {columns.map((column) => (
      ...
      {pagination && totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            {t('table.showing')} {Math.min((currentPage - 1) * itemsPerPage + 1, sortedData.length)} {t('table.to')}{" "}
            {Math.min(currentPage * itemsPerPage, sortedData.length)} {t('table.of')} {sortedData.length} {t('table.entries')}
          </p>
          <div className="flex items-center space-x-2">
            <Button
              variant="outline"

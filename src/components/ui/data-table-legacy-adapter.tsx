 ...
 */
export function DataTableLegacyAdapter<T extends Record<string, unknown>>(props: LegacyDataTableProps<T>) {
  ...
  const mappedColumns = useMemo<ColumnDef<T>[]>(() => {
    ...
    return columns.map((col) => {
      ...
      return {
        id: col.id ?? String(col.accessorKey) ?? col.header ?? String(Math.random()),
  ...
  return (
    <TanStackTable
      ...
      pageCount={totalCount ? Math.ceil((totalCount as number) / (pagination?.pageSize ?? 10)) : undefined}
      sorting={sorting}

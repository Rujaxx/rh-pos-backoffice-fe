...
export default function RestaurantsPage() {
  ...
  // Modal hooks
  const { isOpen, editingItem: editingRestaurant, openModal, closeModal } = useModal<Restaurant>()
  const {
    isConfirmationOpen,
    confirmationConfig,
      ...
      accessor: (restaurant: Restaurant) => (
        <div className="flex items-center">
          <div className="relative w-10 h-10">
            <Image
              src={restaurant.logo || `https://ui-avatars.com/api/?name=${encodeURIComponent(restaurant.name.en)}&background=random`}
      ...
      onClick: (restaurant) => {
        openConfirmationModal(
          () => handleDeleteRestaurant(restaurant),
          {
            ...
            description: t('restaurants.deleteConfirmation', { restaurantName: restaurant.name.en }),
  ...
  const handleUpdateRestaurant = async (data: RestaurantFormData) => {
    ...
    try {
      ...
      setRestaurants(prev => prev.map(restaurant =>
        restaurant._id === editingRestaurant._id ? updatedRestaurant : restaurant
      ))
  ...
  const handleDeleteRestaurant = async (restaurantToDelete: Restaurant) => {
    ...
    try {
      ...
      setRestaurants(prev => prev.filter(restaurant => restaurant._id !== restaurantToDelete._id))
  ...
  const getModalTitle = () => {
    return editingRestaurant
      ? t('restaurants.form.editTitle', { restaurantName: editingRestaurant.name.en })

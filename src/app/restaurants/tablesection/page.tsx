...
export default function TableSectionPage() {
    ...
    const [tableSections, setTableSections] = useState<TableSection[]>(mockTableSections)
    const [loading, setLoading] = useState(false)

    // Modal hooks
    const { isOpen, editingItem: editingTableSection, openModal, closeModal } = useModal<TableSection>()
    const {
        isConfirmationOpen,
        confirmationConfig,
            ...
            accessor: (tableSection: TableSection) => (
                <div>
                    <div className="font-medium text-foreground truncate">
                        {tableSection.restaurantName[locale] || tableSection.restaurantName.en}
                    </div>
                </div>
            ),
            sortable: true
    ...
    const actions: TableAction<TableSection>[] = [
        ...
        },
        {
            ...
            onClick: (tableSection) => {
                openConfirmationModal(
                    () => handleDeleteTableSection(tableSection),
                    {
                        title: t('tableSection.deleteConfirmationTitle'),
                        description: t('tableSection.deleteConfirmationDescription', { name: tableSection.name[locale] }),
    ...
    const handleCreateTableSection = async (data: TableSectionFormData) => {
        ...
        try {
            ...
            const newTableSection: TableSection = {
                ...
                deletedAt: new Date(),
                isActive: true,
                restaurantName: mockTableSections.find(r => r.restaurantId === data.restaurantId)?.restaurantName ||
                    { en: "Unknown Restaurant", ar: "مطعم غير معروف" }
            }
    ...
    const handleUpdateTableSection = async (data: TableSectionFormData) => {
        ...
        try {
            ...
            setTableSections(prev => prev.map(section =>
    ...
    const handleDeleteTableSection = async (tableSectionToDelete: TableSection) => {
        setLoading(true)
        try {
            // Simulate API call
            await new Promise(resolve => setTimeout(resolve, 1000))

            setTableSections(prev => prev.filter(section => section.id !== tableSectionToDelete.id))

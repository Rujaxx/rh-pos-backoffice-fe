...
import {
  TableFormContent,
  useTableForm,
} from '@/components/restaurants/table-form';
...
export default function TablePage() {
    ...
    },
    {
      ...
      onClick: (table) =>
        openConfirmationModal(() => handleDeleteTable(table), {
          ...
          description: t('table.deleteConfirmationDescription', {
            name: table.label,
          }),
  ...
  const handleCreateTable = async (data: TableFormData) => {
    ...
    try {
      ...
      const count =
        data.isBulk && data.bulkCount ? parseInt(data.bulkCount) : 1;
      ...
      const newTables: Table[] = Array.from({ length: count }, (_, i) => ({
        ...
        createdBy: '',
        updatedBy: '',
        restaurantName: mockTables.find(
  ...
  const getModalTitle = () =>
    isEditing ? t('table.edit') : t('table.form.createTitle');
  const getModalDescription = () =>
    isEditing ? '' : t('table.form.createDescription');

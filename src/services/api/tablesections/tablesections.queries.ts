...
import { TableSection, TableSectionFormData, TableSectionQueryParams } from '@/types/tablesection.type';
import { PaginatedResponse, SuccessResponse } from '@/types/api';
import { API_ENDPOINTS, QUERY_KEYS } from '@/config/api';

// Table Section service extending base service
class TableSectionService extends BaseApiService<TableSection, TableSectionFormData, TableSectionFormData> {
  constructor() {
    super(API_ENDPOINTS.TABLE_SECTIONS.LIST);
  }

  // Get all table sections with optional filters
  async getAllTableSections(params?: TableSectionQueryParams): Promise<PaginatedResponse<TableSection>> {
    return this.getAll(params);
  }

  // Get table section by ID
  async getTableSectionById(id: string): Promise<SuccessResponse<TableSection>> {
    return this.getById(id);
  }

  // Get active table sections only (for dropdowns)
  async getActiveTableSections(params?: Omit<TableSectionQueryParams, 'isActive'>): Promise<PaginatedResponse<TableSection>> {
    return this.getAll({ ...params, isActive: 'true' });
  }

  // Get table sections by restaurant ID
  async getTableSectionsByRestaurant(restaurantId: string, params?: Omit<TableSectionQueryParams, 'restaurantId'>): Promise<PaginatedResponse<TableSection>> {
...
export const useGetTableSections = (
  ...
  options?: Omit<UseQueryOptions<PaginatedResponse<TableSection>>, 'queryKey' | 'queryFn'>
) => {
  return useQuery({
...
export const useGetTableSection = (
  id: string,
  options?: Omit<UseQueryOptions<SuccessResponse<TableSection>>, 'queryKey' | 'queryFn'>
) => {
  return useQuery({
...
export const useGetTableSectionsByRestaurant = (
  ...
  return useQuery({
    ...
    queryFn: () => tableSectionService.getTableSectionsByRestaurant(restaurantId, params),

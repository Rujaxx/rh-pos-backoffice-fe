...
export const useCreateTableSection = (
  options?: UseMutationOptions<SuccessResponse<TableSection>, Error, TableSectionFormData>
) => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.data) {
        queryUtils.setQueryData(QUERY_KEYS.TABLE_SECTIONS.DETAIL(data.data._id), data);
      }
    },
    onError: (error) => {
      // Show error message
...
export const useUpdateTableSection = (
  options?: UseMutationOptions<SuccessResponse<TableSection>, Error, { id: string; data: TableSectionFormData }>
) => {
  const queryUtils = useQueryUtils();
  
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: TableSectionFormData }) => {

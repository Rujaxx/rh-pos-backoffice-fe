...
export const useUploadImage = (
  options?: UseMutationOptions<SuccessResponse<UploadResponse>, Error, { file: File; options: UploadOptions }>
) => {
...
export const useUploadMultipleImages = (
  options?: UseMutationOptions<SuccessResponse<UploadResponse[]>, Error, { files: File[]; options: UploadOptions }>
) => {
  const queryUtils = useQueryUtils()
    ...
    onSuccess: (data, variables) => {
      ...
      if (data.data) {
        data.data.forEach((upload) => {
          queryUtils.setQueryData(QUERY_KEYS.UPLOAD.IMAGE(upload.key), { data: upload })
        })
      }
    },
    onError: (error) => {

...
class UploadService {
   ...
   */
  async uploadImage(file: File, options: UploadOptions): Promise<SuccessResponse<UploadResponse>> {
   ...
   */
  async deleteTemporaryUpload(key: string): Promise<DeleteTemporaryUploadResponse> {
    return api.delete(`${this.baseEndpoint.DELETE_TEMPORARY}?key=${encodeURIComponent(key)}`)
  }
}

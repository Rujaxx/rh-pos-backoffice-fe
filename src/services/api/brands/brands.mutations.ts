    ...
    mutationFn: async (data: BrandFormData) => {
      ...
      if (uploadKeys.length > 0) {
        try {
          ...
          console.error('Failed to confirm uploads, but brand was created:', error)
...
export const useUpdateBrand = (
  options?: UseMutationOptions<SuccessResponse<Brand>, Error, { id: string; data: BrandFormData }>
) => {
  const queryUtils = useQueryUtils()
    ...
    mutationFn: async ({ id, data }: { id: string; data: BrandFormData }) => {
      ...
      if (uploadKeys.length > 0) {
        try {
          ...
          console.error('Failed to confirm uploads, but brand was updated:', error)
          // Don't fail the entire operation if upload confirmation fails
        }
      }

...
class TableService extends BaseApiService<Table, TableFormData, TableFormData> {
  ...
  async getAllTables(params?: TableQueryParams): Promise<PaginatedResponse<Table>> {
  ...
  async getAvailableTables(params?: Omit<TableQueryParams, 'isAvailable'>): Promise<PaginatedResponse<Table>> {
    return this.getAll({ ...params, isAvailable: 'true' })
  }

  // Get tables by restaurant ID
  async getTablesByRestaurant(restaurantId: string, params?: Omit<TableQueryParams, 'restaurantId'>): Promise<PaginatedResponse<Table>> {
    return this.getAll({ ...params, restaurantId })
  }

  // Get tables by table section ID
  async getTablesBySection(tableSectionId: string, params?: Omit<TableQueryParams, 'tableSectionId'>): Promise<PaginatedResponse<Table>> {
    return this.getAll({ ...params, tableSectionId })
  }

  // Get tables with minimum capacity
  async getTablesByCapacity(capacity: number, params?: Omit<TableQueryParams, 'capacity'>): Promise<PaginatedResponse<Table>> {
...
export const useTables = (
  ...
  options?: Omit<UseQueryOptions<PaginatedResponse<Table>>, 'queryKey' | 'queryFn'>
) => {
  return useQuery({
...
export const useTable = (
  id: string,
  options?: Omit<UseQueryOptions<SuccessResponse<Table>>, 'queryKey' | 'queryFn'>
) => {
  return useQuery({
